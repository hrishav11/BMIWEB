...
//Now I have added the formula for calculating BMI in "bmi"
  ...
  //When the BMI is less than 18.5, you can see the text below
    if(bmi < 18.5){
        category = "Underweight!! increase muscle mass.";
        result.style.color = "#ffc44d";
    }
  
  //If BMI is >=18.5 and <=24.9
    else if( bmi >= 18.5 && bmi <= 24.9 ){
        category = "Normal Weight!!! Perfect";
        result.style.color = "#0be881";
    }
  
  //If BMI is >= 25 and <= 29.9 
    else if( bmi >= 25 && bmi <= 29.9 ){
        category = "Overweight. lose weight and keep track of calorie intake";
        result.style.color = "#ff884d";
    }
  
  //If BMI is <= 30
    else{
        category = "Obese! high risk factors";
